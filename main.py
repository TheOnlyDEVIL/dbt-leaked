...
class Discord_Bot:
    ...
    def commands(self):
        ...
        async def on_ready():
            ...
            await self.bot.change_presence(activity=discord.Game(name=f"dbt"))
                ...
                if target_id != False:
                    ...
                    for role_name in reversed(config['tfollow']):
                        if discord.utils.get(interaction.guild.roles, name=role_name) in interaction.user.roles:
                            ...
                            embed = discord.Embed(color=embed_color, description=f"```DBT Followers -> {follow_count}```")
                            embed.set_image(url=banner)
                            await interaction.response.send_message(embed=embed)
                            return
                else:
        ...
        async def nuke(interaction, channel: discord.TextChannel):
            if interaction.user.guild_permissions.administrator:
                ...
                if nuke_channel is not None:
                    ...
                    embed = discord.Embed(color=embed_color, description=f"```DBT Nuke```")
                    embed.set_image(url=banner)
